...
// }
export async function getFeaturedMovie(): Promise<featuredMovieDetails | unknown> {
...
export async function getTrendingMovies(): Promise<featuredMovieDetails[] | unknown> {
...
export async function getTrendingTVShows(): Promise<tvShowsDetails[] | unknown> {
  ...
  try {
    ...
    const data = await res.json();
    if (!data.result || data.result.lenght === 0) {
      return null;
    }
  } catch (error) {
    console.error("Failed to fetch featured movie details:", error);
